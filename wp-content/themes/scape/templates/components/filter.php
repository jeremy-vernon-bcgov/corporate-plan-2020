...
if ( sizeof($defaults) > 1 && $filter === 'multi' ) {
	foreach ( $defaults as $key => $category ) {
		if ( 'post' === $post_type ) {
			...
			$category = get_term( $category, 'portfolio_category' )->name;
		}
	...
	if ( 'post' === $post_type ) {
		...
		$category = get_term( $defaults[0], 'portfolio_category' )->name;
	}
	...
	<div class="wtbx-filter-inner row-inner">
		<ul class="wtbx-filter-buttons clearfix">
			<?php if ( $filter === 'minimal' || $filter === 'slider' ) : ?>
				<li class="wtbx-filter-button show-all" data-filter="*">
			...
			<?php if ( isset($filter_categories) ) {
				foreach ( $filter_categories as $cat ) {
					...
					?>
                    <li class="wtbx-filter-button" data-filter="<?php echo esc_attr($cat->slug); ?>">
