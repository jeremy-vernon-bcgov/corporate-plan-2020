...
if ( ! function_exists( 'wtbx_image_smart_img' ) ) {
	function wtbx_image_smart_img( $img_id, $src_size, $srcset_size, $alt = '', $classes = '', $return = false, $lazy = '' ) {
		...
		if ( $is_external ) {
			$image = '<img class="wtbx-lazyloaded" src="' . $img_id . '"/>';
		} elseif ( $img_id ) {
			...
			if ( $src && !empty($src) ) {
				...
				foreach ( $candidates as $candidate ) {
					$size_string = str_replace('-', '', explode(str_replace(array('http://www.', 'https://www.', 'https://', 'http://'), '', $data_path), $candidate)[1]);
					$dims[] = $size_string;
				}
			}
...
if ( ! function_exists( 'wtbx_image_smart_bg' ) ) {
	function wtbx_image_smart_bg( $img_id, $src_size, $srcset_size, $ratio = '', $alt = '', $classes = '', $return = false, $lazy = '' ) {
		...
		$lazy = $lazy !== '' ? $lazy : wtbx_option_levelled('site-smartimage');
...
function wtbx_add_footer_styles() {
	...
	wp_register_script( 'scape-main-js', WTBX_URI . '/library/js/app.js', wtbx_script_queue(), SCAPE_VERSION, true );
