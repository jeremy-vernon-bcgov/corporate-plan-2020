...
function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
	...
	<div id="comment-body-<?php comment_ID() ?>" class="comment-body">
		...
		<div class="comment-container clearfix">
			...
			if ( $seconds <= 604800 ) : ?>
				<div class="comment-meta comment-metadata"><?php printf( _x( '%s ago', '%s = human-readable time difference', 'scape' ), human_time_diff( get_comment_time( 'U' ), current_time( 'timestamp' ) ) ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'scape' ) ); ?></div><!-- /.comment-meta -->
			<?php else : ?>
				<div class="comment-meta comment-metadata"><?php echo get_comment_time('j F Y'); ?><?php edit_comment_link( esc_html__( '(Edit)', 'scape' ) ); ?></div><!-- /.comment-meta -->
			<?php endif; ?>
